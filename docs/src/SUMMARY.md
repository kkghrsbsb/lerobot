# Summary

- [项目总览（入口文档）](README.md)

- [reference]()

  - [piper-act-deployment](reference/piper-act-deployment/README.md)
  
    - [01 LeRobot 开发笔记](reference/piper-act-deployment/01-lerobot-development-notes.md)
    - [02 Piper 移植 LeRobot 开发记录](reference/piper-act-deployment/02-piper-porting-lerobot-development-record.md)
    - [03 Piper SDK 接口函数汇总](reference/piper-act-deployment/03-piper_sdk-interface-fn-summary.md)
    - [04 Piper 移植 LeRobot 方案优化](reference/piper-act-deployment/04-piper-porting-lerobot-solution-optimization.md)
    - [05 Piper 移植 LeRobot 运动控制优化](reference/piper-act-deployment/05-piper-porting-lerobot-motion-control-optimization.md)
    - [06 学习 Transformer 机器人（ACT）](reference/piper-act-deployment/06-learning-transformer-robot-throught-ACT.md)
    - [07 基于注意力机制的模仿学习](reference/piper-act-deployment/07-imitation-learning-with-attention-mechanisms.md)
    - [08 Piper ACT 试验记录](reference/piper-act-deployment/08-piper-ACT-trial-record.md)

- [review]()

  - [PiperMotorsBus write 及整体代码审查](review/piper-motors-bus-write-and-general.md)

- [explain]()

  - [PiperMotorsBus 缺陷修复变更说明](explain/piper-motors-bus-bugfix.md)
