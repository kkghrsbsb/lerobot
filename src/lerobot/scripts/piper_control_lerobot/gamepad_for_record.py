"""
检测机械臂是否已使能且在零位（这个操作对应 piper_observe.py 中 connect），然后可以通过手柄遥操
通过独立连接检测+手柄遥操控制机械臂，此脚本专门用作为数据采集

前置条件：启动 piper_observe.py ，其中 connect() 使机械臂进入使能+零位状态。
...
def main():
    print("=" * 60)
    print("通过独立连接检测+手柄遥操控制机械臂，此脚本专门用作为数据采集")
    print("=" * 60)
